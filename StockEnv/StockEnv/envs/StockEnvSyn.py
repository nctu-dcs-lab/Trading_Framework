...
class StockMarketSyn(StockMarketBase):
        ...
        for i in range(2):
            for year in range(self.train_start, self.train_end + 1):
                for month in range(1, 13):
                    if i == 1:
                        file_path = f'synthetic_data/tfe-mtx00-{year}{month:02d}-{self.scalar}min-r.csv'
                    else:
                        file_path = f'synthetic_data/tfe-mtx00-{year}{month:02d}-{self.scalar}min.csv'
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.reverse == 0:
            ...
            file_path = f'synthetic_data/tfe-mtx00-{self.curr_year}{self.curr_month:02d}-{self.scalar}min-r.csv'
    ...
    def save_results(self):
        ...
        if self.reverse == 0:
            ...
            save_path = os.path.join(self.save_dir, f'tfe-mtx00-{self.curr_year}{self.curr_month:02d}-{self.scalar}min-r.csv')
