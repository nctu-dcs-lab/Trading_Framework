...
if __name__ == '__main__':
    ...
    parser.add_argument('--prob', type=float, help='The prob to select a correct trend.')
    ...
    # set env and trader
    env = make_env(args.env, save_dir=f'valid_csv/CombineTrader_trend_{args.trend_len}_prob_{int(args.prob)}_ma_{args.ma}_seed_{args.seed}_DailyMethod', start_year=2021, end_year=2024, train_start=2018, train_end=2020, init_balance=args.balance)
    ...
    for j in range(48):
        ...
        while not done:
            ...
            if time_step == 0 or curr_trader == 0:
                curr_trader = env.get_trader()
                trend_num += 1
                if random.random() < (args.prob/100):
                    env.set_trader(curr_trader)
                else:
                    choice_wrong += 1
                    curr_trader = -curr_trader
                    env.set_trader(curr_trader)
            time_step += 1
            if time_step >= args.trend_len:
                time_step = 0
