...
if __name__ == "__main__":
    ...
    for col in ['close', 'open', 'high', 'low']:
        ...
        print(f'Start denoising {col}.')
        
        denoised_data, noise = wavelet_denoising(df[col])
        df.insert(len(df.columns), f'{col}_denoised', denoised_data)
        df.insert(len(df.columns), f'{col}_noise', noise)
        
        print(f'{col} denoising is completed.')

        print(f'plot denoised data.')
        plot_wavelet_denoising(df, col)
    ...
    save_path = os.path.join(save_dir, f'denoised_data.csv')
